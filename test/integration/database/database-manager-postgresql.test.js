...
import { postgresConfig } from '../setup/database-config.js'
import { randomUUID } from 'crypto'

describe('DatabaseManager PostgreSQL Integration Tests', () => {
  ...
  describe('connect', () => {
    ...
    it('should connect successfully with valid PostgreSQL configuration', async () => {
      // GIVEN - Valid PostgreSQL configuration
      const connectionName = `postgres-test-${randomUUID()}`
      testConnectionNames.push(connectionName)
      const config = { ...postgresConfig, name: connectionName }
    ...
    it('should throw error when invalid credentials provided', async () => {
      ...
      const connectionName = `postgres-invalid-${randomUUID()}`
      testConnectionNames.push(connectionName)
      const invalidConfig = { ...postgresConfig, name: connectionName, password: 'wrongpassword' }
  ...
  describe('executeQuery', () => {
    ...
    beforeEach(async () => {
      // Setup active PostgreSQL connection
      connectionName = `postgres-query-${randomUUID()}`
      testConnectionNames.push(connectionName)
      const config = { ...postgresConfig, name: connectionName }
    ...
    it('should execute simple SELECT query', async () => {
      ...
      const result = await sut.executeQuery(connectionName, 'SELECT $1 as message', ['Hello PostgreSQL'])
    ...
    it('should execute CREATE TABLE and INSERT operations', async () => {
      ...
      await sut.executeQuery(connectionName, `CREATE TEMPORARY TABLE ${tableName} (id SERIAL PRIMARY KEY, name VARCHAR(100))`)

      const insertResult = await sut.executeQuery(connectionName, `INSERT INTO ${tableName} (name) VALUES ($1) RETURNING id`, ['Test Record'])
  ...
  describe('testConnection', () => {
    ...
    it('should return success for healthy connection', async () => {
      ...
      const connectionName = `postgres-health-${randomUUID()}`
      testConnectionNames.push(connectionName)
      await sut.connect({ ...postgresConfig, name: connectionName })
  ...
  describe('getSchema', () => {
    ...
    beforeEach(async () => {
      // Setup active PostgreSQL connection
      connectionName = `postgres-schema-${randomUUID()}`
      testConnectionNames.push(connectionName)
      await sut.connect({ ...postgresConfig, name: connectionName })
    })
  ...
  describe('exportConnections', () => {
    ...
    it('should export active connections', async () => {
      // GIVEN - Active connection
      const connectionName = `postgres-export-${randomUUID()}`
      testConnectionNames.push(connectionName)
      await sut.connect({ ...postgresConfig, name: connectionName })
      ...
      expect(exported[connectionName].type).toBe('postgresql')
  ...
  describe('disconnect', () => {
    ...
    it('should disconnect active connection', async () => {
      // GIVEN - Active connection
      const connectionName = `postgres-disconnect-${randomUUID()}`
      testConnectionNames.push(connectionName)
      await sut.connect({ ...postgresConfig, name: connectionName })
  ...
  describe('disconnectAll', () => {
    ...
    it('should disconnect all connections', async () => {
      ...
      const connectionName1 = `postgres-all1-${randomUUID()}`
      const connectionName2 = `postgres-all2-${randomUUID()}`
      testConnectionNames.push(connectionName1, connectionName2)

      await sut.connect({ ...postgresConfig, name: connectionName1 })
      await sut.connect({ ...postgresConfig, name: connectionName2 })
  ...
  describe('hasConnection and getConnectionCount', () => {
    ...
    it('should track connection state correctly', async () => {
      ...
      const connectionName = `postgres-state-${randomUUID()}`
      testConnectionNames.push(connectionName)
      await sut.connect({ ...postgresConfig, name: connectionName })
